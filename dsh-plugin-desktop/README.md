...
`yarn package:dir` creates an unpacked directory for the current host platform. The packaged-runtime gate rejects an application archive that omits the desktop update and terminal modules, the DSH CLI bootstrap, the bundled pnpm entry, or the physical deployment package. Electron Builder emits the root manifest, desktop runtime, and complete dependency tree under `app.asar.unpacked`; both Host profile boot and the CLI bootstrap use this physical tree so DSH profile-fallback symlinks never target a virtual ASAR directory. Both native icon sources are derived from `assets/parametria-logo-icon.svg`, the one piece of brand artwork this package owns, by `scripts/generate-brand-icons.ts`. That generator is run by hand rather than from `yarn build`, because its outputs are committed source artifacts the build then consumes, and a PNG re-encoded on every machine could carry no digest pin; run it after changing the mark, and the drift guards in `tests/package.spec.ts` fail until you do. No `.ico` or `.icns` is authored here — Electron Builder derives both from the PNGs.

`build/app-icon.png` is the Parametria mark on an opaque plate, and the Windows/Linux application icon. Its digest pin is what says the committed bytes are the reviewed ones; a low-resolution render comparison beside it says those bytes still depict the mark. The build runs `scripts/generate-mac-app-icon.mjs` to center that artwork at 824 by 824 pixels on a transparent 1024 by 1024 canvas; macOS packaging and the live Dock both use the generated `build/app-icon-mac.png`. `build/tray-icon.svg` is the tray source: the mark's solid elements only, flattened to a single colour and cropped to the artwork's own bounding box. The single colour is required rather than stylistic — the build derives a macOS template image by replacing that one colour with black, and a template image carries alpha and nothing else — and the build also derives fixed brand-colour Windows and Linux tray images at four scale factors.
...
- This project pins both the published DSH `0.1.5-rc.2` family and the corresponding official `deepseek-harness/` release source. Product builds still resolve published package interfaces rather than linking the source checkout.
