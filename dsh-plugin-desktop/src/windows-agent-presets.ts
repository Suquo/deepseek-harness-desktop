...
/**
 * The refusal upstream's own `resolve` raises for an id no root supplies, so the
 * hidden preset reads exactly like an absent one to every caller.
 * @param id - the requested preset id.
 * @param available - the ids the Windows-filtered roster does supply.
 * @returns the `agent-preset/not-found` Remote failure.
 */
...
/**
 * The refusal upstream's own `copy` raises for a taken id (its `presetExists`
 * helper is not exported), so the reserved id reads as already taken.
 * @param id - the reserved preset id.
 * @returns the `agent-preset/invalid` Remote failure.
 */
