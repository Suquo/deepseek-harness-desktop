...
| `dsh-subagent` | `lib/index.js` (continuable lifecycle + notice + config + export list) + `lib/types/{index,lifecycle,out-of-process}.d.ts` | the exported `limitSubagentDiagnostic` seam plus a default-off continuable settlement diagnostic: the Parametria profile opts in so a terminal child `turn/end` error reaches the parent as bounded `{code} — {message} (child session {id})` detail, while compatibility composition retains upstream's bare notice | **RE-VALIDATE:** all 14 context-bearing hunks, the default-off config and Parametria profile row, both error shapes, the 4096-byte UTF-8 bound, the clean-completion negative case, and the compatibility bare-notice case in `subagent-error-surface.spec.ts`. Also re-check the one-shot driver's import/export pairing — this patch's pair is `dsh-subagent-in-process-driver`, and the two must move together or the driver's import resolves to nothing. Applied clean at 0.1.1-rc.2. |
