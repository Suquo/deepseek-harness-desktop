...
| `pi-ai@0.82.1` (#60) | `dist/api/openai-completions.js` | **provider wire** — a bare OpenRouter route must send no `reasoning` field when nothing selects an effort | **Not** harness-versioned, but not independent of the pin either: `@earendil-works/pi-ai` arrives TRANSITIVELY through `@deepseek-ai/dsh-llm-pi-ai`, so a harness bump can move it while the patch filename still names the old version. Its single selector is a caret (`npm:^0.82.1`); when the transitive range moves the selector matches nothing, the patch applies to nothing, and `yarn install` still succeeds. Since #60 `check:layout` fails on exactly that (a patch with resolutions entries but no `patch:` locator in the lockfile), and `dsh-plugin-desktop/tests/pi-ai-bare-route-reasoning.spec.ts` fails on the behavior. Target path is stable, not hash-named. Charter rule: a wire change is not resolved by a green gate — the issue stays open pending a live provider datum. |
