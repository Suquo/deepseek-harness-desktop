...
5. **Live-incident diagnosis** — toolbox: the packaged/dev app's logs and evidence files under the DSH user-data dir; port owners via `Get-NetTCPConnection` (the Host binds an ephemeral loopback port — find it by PID); `corepack yarn check` as the full headless gate. Ground every report at file:line + ACTUAL DATA before filing; Q&A-settle interaction ambiguity BEFORE filing.
