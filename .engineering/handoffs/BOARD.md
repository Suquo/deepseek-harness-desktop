...
- **Lane B (parametria-harness, `pm/*`)**: **BUSY — gen-12 on #24** (command-level policy via tools/pre-execute; grants keyed command-shape+cwd; branch `pm/issue-24-command-policy`). SURFACE: preset + desktop plugin are Lane B's; freezes rather than touching patches/root manifest. Queue after: #23 → #52 → #54 → #58; #30/#45 available, #6 buildable, #7 owner-gated.
