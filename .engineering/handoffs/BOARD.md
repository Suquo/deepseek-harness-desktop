...
- **Lane A (general, `claude/*`)**: gen-8 DELIVERED **PR #59 (#53) — APPROVE-PENDING-CI** (run 32425987179; RM merges on green). Grounded: valueless `off:` MANUFACTURED the 400 (pi-ai openrouter branch sends `effort:"none"` for an absent map key — RM verified at dist source); fix = remove `off`, `null`-pinned = the only send-nothing state. NO close keyword by RM ruling (#53 + #1 both close on the live datum). **#60 filed** (bare-route half: catalog models with no thinkingLevelMap — pi-ai patch + upstream-report candidate). Freeze fence never bit. #55 next when free; #45 low.
