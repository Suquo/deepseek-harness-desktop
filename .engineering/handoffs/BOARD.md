...
- **Lane A (general, `claude/*`)**: FREE — gen-8 delivered #53's preset half (**PR #59 MERGED on green CI, 2026-08-21 ~01:04**, all 5 jobs): valueless `off:` removed from the managed route (it MANUFACTURED the 400 — pi-ai openrouter branch sends `effort:"none"` for an absent map key; RM verified at dist source). **RM ran install:profile post-merge — the live block is fixed** (`reasoningEfforts: high` only). #53 + #1 close together on ONE live datum: operator restarts the app and runs once. **#60 filed** (bare-route half). Next: #55; #45 low.
