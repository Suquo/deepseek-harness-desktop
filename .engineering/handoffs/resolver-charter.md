...
- Toolchain on cjjmaster (Linux, 2026-09-02): mise Node 26 + Corepack 0.36 + Yarn 4.18.0; Electron userData defaults to `~/.config/DSH Desktop`. Linux is NOT an upstream-supported distributable (no AppImage/deb target; `yarn package:dir` yields `dist/linux-unpacked` only) — Windows packaging tests still run in CI on `desktop-windows`, so a Windows-only regression is caught there, not locally. Operator-visible lane sessions run in Herdr tabs (w4:p2 Lane A, w4:p3 Lane B); RM-spawned background agents remain the mechanism for Lanes C/D.
