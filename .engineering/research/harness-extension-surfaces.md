...
| **Desktop fork's own layer** | `dsh-plugin-desktop/cordis.patch.yml` (38 lines) — inserts desktop plugins then id-patches `web-runtime` config, restating every field the pinned bundle row sets (see the gotcha below; #49 was the omission of one). Pure patch layer; no upstream source edits. | `dsh-plugin-desktop/cordis.patch.yml` |
