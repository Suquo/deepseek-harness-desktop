...
- Compatibility mode must run the upstream default client without BEHAVIOR overrides. **Visual branding overrides (styles, marks, wordmarks, titles/icons) are permitted in compatibility mode** (owner ruling 2026-08-20, recorded on issue #26 — "brand everywhere"); they must stay presentation-only and never alter slots, services, or behavior there. Advanced presentation beyond branding belongs to desktop-owned client plugins and may replace documented slots or services through profile composition.
